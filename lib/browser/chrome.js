...
export default class Chrome extends Browser {
    ...
    async stopRecordNetworkLog (dist = path.join(process.cwd(), 'network.log.har')) {
        ...
        await this.devtools.Network.disable()
        return har
    }

    async profileCPU () {
        /**
         * don't start profiling if already started
         */
        if (this.cpuProfilingEnabled) {
            return
        }

        log.info('Enable cpu profiling')
        this.cpuProfilingEnabled = true
        await this.devtools.Profiler.enable()
        await this.devtools.Profiler.start()
    }

    async stopCPUProfiling (dist = path.join(process.cwd(), 'page.cpuprofile')) {
        const profile = await this.devtools.Profiler.stop()
        fs.writeFileSync(dist, JSON.stringify(profile.profile))
        return profile
    }
